'use strict';
const { app, BrowserWindow, Menu, clipboard, ipcMain, dialog, webContents, safeStorage } = require('electron');
const fs = require('fs');
...
function safeFileName(url, mime, index) {
  ...
  try {
    const u = new URL(url);
    name = decodeURIComponent(path.basename(u.pathname || ''));
  } catch {}
  ...
  state.handler = async (_event, method, params) => {
    try {
      if (method === 'Network.requestWillBeSent') {
        state.rows.set(params.requestId, {
        ...
        Object.assign(row, {
          url: row.url || params.response?.url || '',
        ...
        try {
          ...
          if (estimated <= 25 * 1024 * 1024 && state.bytes + estimated <= 200 * 1024 * 1024) {
            row.body = body.body;
            row.base64Encoded = !!body.base64Encoded;
            state.bytes += estimated;
          } else {
            row.bodySkipped = 'too-large';
          }
        } catch (error) {
          row.bodySkipped = String(error?.message || error);
        }
...
async function stopCapture(contentsId) {
  ...
  const out = path.join(dir, `KBrowser_Network_${stamp}`);
  ...
  for (const row of state.rows.values()) {
    ...
    if (row.body != null) {
      ...
      fs.writeFileSync(path.join(resDir, fileName), buffer);
      copy.savedFile = `resources/${fileName}`;
    }
    manifest.push(copy);
  }
